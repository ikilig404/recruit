...
import fxb.java.recruit.entity.CommentEntity;
import fxb.java.recruit.entity.FavorEntity;
import fxb.java.recruit.entity.UserEntity;
import fxb.java.recruit.mapper.*;
...
public class RecPositionUtil {
    ...
    public ArrayList<PositionCompanyBO> recommend(HashMap<Integer, Integer> map, UserEntity user) {
        ...
        System.out.println(resumeId + "resumeIdresumeIdresumeIdresumeIdresumeIdresumeIdresumeIdresumeId!!!");
        //应聘记录
        ...
        activation = (valuePos + valueCom + valueCol) / (listPos.size() + listCom.size() + listCol.size());
        System.out.println(activation + "activationactivationactivationactivationactivation");
        ArithmeticUtil recArithmetic = new ArithmeticUtil();

        //根据活跃度标准判断调用的推荐算法
        if (activation < actStandard) {
            System.out.println("基于流行度算法0000000000000000000000000");
            posList = recArithmetic.popularityRec(map, user);
        } else {
            System.out.println("基于协同过滤算法222222222222222222222222");
