...
public interface HRMapper {
    ...
    @Insert("insert into hr(hrMobile,hrPassword,hrName,hrEmail,departmentId) values(#{hrMobile},#{hrPassword},#{hrName},#{hrEmail},#{departmentId})")
    int saveHR(HREntity hrEntity);

    @Select("select * from hr where hrMobile = #{hrmobile} limit 1")
    HREntity getHRByMobile(@Param("hrmobile") String moblie);
