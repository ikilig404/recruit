package fxb.java.recruit.controller.user;
...
public class DataController extends BaseController {
    ...
    public String index(HttpSession httpSession, @PathVariable int page, @RequestParam(value = "limit", defaultValue = "6") int limit) {
        ...
        if (user == null) {
            List<PositionCompanyBO> posInfo = positionService.listAllPosCompany();
...
//        }
     ...
     * @return
     */
    @GetMapping(value = "/user/applyOrNot/{id}")
    @ResponseBody
    public int applyOrNot(HttpServletRequest request, @PathVariable int id) {
        ...
        if (applicationService.applyOrNot(id, resume.getResumeId())) {
            return 1;
        } else {
            return 0;
        }
    ...
    /**
     * 根据positionId和resumeId删除职位申请记录
     * @param request
     * @param id
     * @return
     */
    @GetMapping(value = "/user/disapply/{id}")
    @ResponseBody
    public String disapply(HttpServletRequest request, @PathVariable int id) {
        ...
        boolean result = applicationService.removeApplication(resume.getResumeId(), id);
    ...
    /**
     * 根据applicationId删除职位申请记录
     * @param id
     * @return
     */
    @GetMapping(value = "/user/deleteapply/{id}")
    @ResponseBody
    public String deleteApply(@PathVariable int id) {
        boolean result = applicationService.removeApplicationById(id);
...
//        UserEntity user = this.getUser(request);
    ...
    public String userLogout(HttpServletRequest request) {
        ...
        request.getSession().removeAttribute(GlobalConst.LOGIN_USER_KEY);
